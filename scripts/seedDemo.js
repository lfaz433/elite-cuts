...
// 1. Initialize Firestore Admin client via OAuth2Client using CLI token
...
// 2. Initialize Firebase Web SDK for Auth operations
...
async function clearDemoData() {
  ...
  for (const col of collections) {
    const snap = await db.collection(col).where('tenantId', '==', TENANT_ID).get();
...
async function createAuthUser(email, password, displayName) {
  try {
    ...
    console.log(`  ✅ Created new user: ${email} (UID: ${cred.user.uid})`);
    return cred.user.uid;
  } catch (err) {
    if (err.code === 'auth/email-already-exists') {
      const cred = await signInWithEmailAndPassword(clientAuth, email, password);
      console.log(`  ✅ User already exists, signed in: ${email} (UID: ${cred.user.uid})`);
      return cred.user.uid;
    }
    throw err;
  }
...
async function main() {
  ...
  for (const b of barbersData) {
    const docRef = db.collection('barbers').doc();
    await docRef.set({
      ...b,
      tenantId: TENANT_ID,
      image: '',
  ...
  for (const e of expenses) {
    ...
    await docRef.set({ ...e, tenantId: TENANT_ID, createdAt: Date.now() });
